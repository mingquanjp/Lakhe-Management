...
CREATE TABLE households (
    ...
    status VARCHAR(20) DEFAULT 'Active' CHECK (status IN ('Active', 'MovedOut')) -- 'Active', 'MovedOut'
);
...
CREATE TABLE residents (
    ...
    notes TEXT, -- Ghi chú
    status VARCHAR(20) DEFAULT 'Permanent' CHECK (status IN ('Permanent', 'MovedOut', 'Deceased')) , -- 'Permanent', 'MovedOut', 'Deceased'
...
CREATE TABLE temporary_residents (
    ...
    host_household_id INT NOT NULL, -- Chủ hộ
    first_name VARCHAR(50) NOT NULL, -- Họ
    last_name VARCHAR(50) NOT NULL, -- Tên
    identity_card_number VARCHAR(20), -- CMND/CCCD
    dob DATE, -- Ngày sinh
    gender VARCHAR(10) CHECK (gender IN ('Male', 'Female')), -- 'Male', 'Female'
    home_address VARCHAR(255), -- Địa chỉ thường trú ở quê
    reason TEXT, -- Lý do
...
CREATE TABLE fees (
    ...
    amount DECIMAL(15, 2), -- Số tiền (NULL nếu là tự nguyện đóng góp tùy tâm)
    start_date DATE NOT NULL, -- Ngày bắt đầu
    end_date DATE -- Ngày kết thúc
);
...
CREATE TABLE payment_history (
    ...
    fee_id INT NOT NULL,
    household_id INT NOT NULL, 
    amount_paid BIGINT NOT NULL, -- Số tiền nộp
