...
const FeeDashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    return [
      {
        value: formatMoney(overallStats.total_paid),
        unit: "VND",
        label: "Tổng tiền đã thu",
      },
      {
        value: formatMoney(overallStats.expected_total),
        unit: "VND",
        label: "Tổng tiền thu dự kiến",
      },
      {
        value: (overallStats.total_households || 0).toString(),
        unit: "",
        label: "Tổng số hộ khẩu",
      },
      {
        value: (overallStats.active_fees || 0).toString(),
        unit: "",
        label: "Số khoản thu đang hoạt động",
      },
    ];
  }, [overallStats]);
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      render: (value) => (
        <span
          className={`status-badge ${value === "paid" ? "status-paid" : "status-unpaid"
            }`}
        >
